...
def symbol_table():
    ...
    # 내부의 __dict__를 확인하면 해당 객체 내부의 심볼 테이블을 확인할 수 있음
    # 런타임에 속성, 메서드를 추가할 때
...
import sys  # 원래는 위에 다 몰아서 작성해야 하지만 지금은 흐름상 여기에 씀

def ref_count():
    """
    객체가 참조될 때 참조 카운트가 증가되면서 관리
    더이상 참조되는 카운트가 없을 때  Garbage Collector 실행
    """
    x = object()
    print(type(x))
...
def object_id():
    ...
    lst1 = [1, 2, 3]
    lst2 = [1, 2, 3]
    print("lst:", hex(id(lst1)), hex(id(lst2))) # 가변 자료형 -> 값이 같아도 다른 객체를 참조한다

    # == : 동등성의 비교, is : 동일성의 비교
    print(i1 == i2, i1 is i2)           # int
    print(s1 == s2, s1 is s2)           # str
    print(lst1 == lst2, lst1 is lst2)   # list (값이 같아도(동등) 다른 객체를 참조한다(동일하지 않음))

    # is 연산자는 id의 비교
    print(id(lst1) == id(lst2), lst1 is lst2)   # 객체 id를 비교해봤을 때 False
