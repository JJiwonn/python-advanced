...
print(sum_val(2,3))
print(incr(10)) # 두번째 인수가 부여되지 않으면 기본값을 활용됨
print(incr(10, 2))  # 기본값을 무시하고 값을 부여하면 그 값이 활용됨

# 키워드 인수
def area(width, height):
    return width * height

print(area(10, 20)) # 기존 방식대로 설계된 매개변수 순서대로 호출
print(area(width=10, height=20))    # 매개변수의 이름값으로 호출
print(area(height=20, width=10))    # 매개변수 호출순서는 중요하지 않음

# 가변 인수 : 정해지지 않은 수의 매개변수 -> 매개변수 앞에 * 붙인다
...
# 키워드 인수 : **
# 함수에 고정인수, 가변인수, 키워드 인수가 있을 때
# 선언부에 고정인수, 가변인수, 키워드 인수 순서로 선언해 줘야 한다
...
func_args(10, 20, 30, 40, 50, 60, 70, option="test", option2="kwargs")
